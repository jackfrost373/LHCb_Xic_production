...
pidcuts["K"]="[MC12TuneV2_ProbNNK > 0.4]"
pidcuts["Pi"]="[MC12TuneV2_ProbNNpi > 0.5]"
pidcuts["P"]="[MC12TuneV2_ProbNNp > 0.5]"
#pidcuts["K"]="[MC15TuneV1_ProbNNK > 0.4]"
#pidcuts["Pi"]="[MC15TuneV1_ProbNNpi > 0.5]"
#pidcuts["P"]="[MC15TuneV1_ProbNNp > 0.5]"
...
do :
  ...
  do :
    ...
    do :
      ...
      python PIDCalib/PIDPerfScripts/scripts/python/MultiTrack/MakePerfHistsRunRange.py "${sample}" "${magnet}" "${particle}"  "${pidcuts[$particle]}" "${varx}" "${vary}" "${varz2}" -c "${precuts}"
    done
  done
done
