...
def strip_n_save (min, max, cuts, directory, saving_directory, extra_variables, particle = "Lc", bins = False, y = True, ybin = [0,0], ptbin = [0,0], tree = None):
    ...
    #Check if there are any issues with the data
        ...
        if not (tree == None):
            alldata = tree
        if y:
            extra_string = "_ybin_{0}-{1}.root".format(ybin[0], ybin[1])
        else:
            extra_string = "_ptbin_{0}-{1}.root".format(ptbin[0], ptbin[1])
