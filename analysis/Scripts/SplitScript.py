...
pbins = [ [3000,4000],[4000,5000], [5000,6000], [6000,7000], [7000,8000], [8000,9000], [9000,10000], [10000,12000],[12000,15000],[15000,20000]]
...
for particle in particles:
    ...
    for ybin in ybins:
        for pbin in pbins:
            yptcut = "p >= {0} && p < {1} && y >= {2} && y < {3}".format(pbin[0], pbin[1], ybin[0], ybin[1])
            allcuts = "{0} && {1} && {2}".format(globalCuts, yptcut, mass_cuts)
            wfile = TFile.Open(directory + particle + "_splitfile_y{0}-{1}_p{2}-{3}.root".format(pbin[0],pbin[1],ybin[0],ybin[1]), "RECREATE")
