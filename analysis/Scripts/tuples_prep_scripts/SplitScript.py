...
def split_in_bins_n_save (root_file, saving_dir, run, mother_particle = "Lc"):
    ...
    for particle in particles:
        ...
        for ybin in ybins:
            ...
            Strip.strip_n_save(0,0, allcuts, "", saving_dir + "ybins/" + particle, extra_variables = [""], bins = True, y =True, ybin = ybin, tree = tree)
        for ptbin in ptbins:
            ptcuts = "lcplus_PT >= {0} && lcplus_PT < {1}".format(ptbin[0], ptbin[1])
            allcuts = " {0} && {1}".format(ptcuts, mass_cuts)
            Strip.strip_n_save(0,0, allcuts, "", saving_dir + "ptbins/" + particle, y_2017=False, extra_variables = [""], bins = True, y =False, ptbin = ptbin, tree = tree)
