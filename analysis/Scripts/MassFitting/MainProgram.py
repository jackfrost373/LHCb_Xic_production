		...
		for j in magPol:
			...
			for filename in os.listdir(directory + str(i) + "_" + j + "/bins/"):
				...
				if filename.startswith("Lc"):
					...
					Shape_fit("GaussCB", MClocation, filename, mainDict, i, j, "Lc", fittingDict)
					
				if filename.startswith("Xic"):
					MClocation = os.path.join(newDir,filename)
					Shape_fit("GaussCB", MClocation, filename, mainDict, i, j, "Xic", fittingDict)
...
def Shape_fit(shape, MClocation, filename, mainDict, i, j, particle, fittingDict):

	fullname = str(i) + "_" + j + "_" + filename
