...
name2 = tree.GetName() + ""
tree.Draw(varname+">>masshist_Lc("+str(nbins)+","+str(2240)+","+str(2340)+")", DataCuts)
masshist_Lc = ROOT.gDirectory.Get("masshist_Lc")

masshist_Lc.SetTitle("Mass Fit of Lc")
masshist_Lc.GetYaxis().SetTitle("Number of events")
masshist_Lc.SetLineColor(4)
...
exponential_Lc = ROOT.RooRealVar("exponential_Lc","C", 0.001, -0.2, 0.2)
...
gauss_Norm_Lc  = ROOT.RooRealVar("gauss_Norm_Lc","Gauss Yield Lc", tree.GetEntries()/nbins * 3, 0, tree.GetEntries() * 2)
cb_Norm_Lc     = ROOT.RooRealVar("cb_Norm_Lc","CB Yield Lc", tree.GetEntries()/nbins * 3/10, 0, tree.GetEntries() * 2)
...
Actual_signalshape_Norm = ROOT.RooRealVar("Actual_signalshape_Norm","Signal Yield", tree.GetEntries()/nbins * 3/10, 0, tree.GetEntries() * 2)
