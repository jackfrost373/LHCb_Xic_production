...
from PID_eff_Dict import PIDDict
...
def plotEff(Dict,title,range_yMin,range_yMax):
    ...
    for year in years:
        ...
        Lc_err_md.append(Dict["Lc_{}_MagDown".format(year)]["err"])
        Lc_mu.append(Dict["Lc_{}_MagUp".format(year)]["val"])
        Lc_err_mu.append(Dict["Lc_{}_MagUp".format(year)]["err"])

        Xic_md.append(Dict["Xic_{}_MagDown".format(year)]["val"])
        Xic_err_md.append(Dict["Xic_{}_MagDown".format(year)]["err"])
        Xic_mu.append(Dict["Xic_{}_MagUp".format(year)]["val"])
        Xic_err_mu.append(Dict["Xic_{}_MagUp".format(year)]["err"])
...
def main(argv):
    ...
    for opt in options:
        if opt == "-s":
            plotEff(selDict,"Selection Efficiency vs. Year",0.0,0.18)

        elif opt == "-t":
            plotEff(trigDict, "Trigger Efficiency vs. Year",0.4,0.9)

        elif opt == "-p":
            plotEff(PIDDict, "PID Efficiency vs. Year",0.4,0.9)
