...
folders_dict = {"42":["2012_MagDown", 1155],  "46":["2012_MagUp", 1342]} #a dictionary containing the details of the all the years' data according to joblog.txt
...
if not os.path.exists(outputdir+"2012"):
  os.mkdir(outputdir + "2012")
...
for particle_type in particle_types :
    ...
    for y_bin in y_bins :
      ...
      for pt_bin in pt_bins :
        ...
        for element in folders_dict :
          ...
          tree.Add(inputdir+name+"/bins/"+particle_type+"_bin"+y_bin+pt_bin+".root")
          cuts = "1==1"
  ...
  if(makesWeights) :
    ...
    frame.Draw()
    c1.Update()
    c1.SaveAs("{0}/{1}_sWeight_fit.pdf".format(outputdir+"2012", "all"+particle_type))
    ...
    for var in [gauss_mean, gauss_width, cb_width, cb_alpha, cb_n, sigfrac, exponent] :
      ...
      ws.writeToFile("{0}/{1}_sWeight_ws.root".format(outputdir+"2012",particle_type))
  ...
  if(makeFriendTree) :
    ...
    wfile = ROOT.TFile.Open("{0}/{1}_sWeight_swTree.root".format(outputdir+"2012",particle_type),"RECREATE")
  ...
  if(plotVariable) :
    ...
    fws = ROOT.TFile.Open("{0}/{1}_sWeight_ws.root".format(outputdir+"2012",particle_type),"READONLY")
    ws = fws.Get('ws')
    var = ws.var(variable)
    data = ws.data("swdata")
    ...
    c2.Update()
    c2.SaveAs("{0}/{1}_sPlot_{2}.pdf".format(outputdir+"2012",particle_type,variable))
  ...
  if(testFriendTree) :
    ...
    if particle_type =="Lc": 
      ...
      datacuts += " && 1==1"
    if particle_type =="Xic": 
      #datacuts += " && lcplus_MM >= 2240 && lcplus_MM <= 2340 && lcplus_P >= 5000 && lcplus_P <= 200000 && lcplus_TAU >= 0 && lcplus_TAU <= 0.007"
      datacuts += " && 1==1"
    print("beginning CopyTree")
    wfile = ROOT.TFile.Open(outputdir+"2012"+"/"+particle_type+"cuttree.root","RECREATE")
    ...
    cuttree.AddFriend("swTree","{0}/{1}_sWeight_swTree.root".format(outputdir+"2012",particle_type))
    ...
    c4.Update()
    c4.SaveAs("{0}/{1}_sPlot_swTree_{2}.pdf".format(outputdir+"2012",particle_type,var))
